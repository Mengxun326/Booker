...
namespace booker
{
	...
	std::vector<EntryInfo> ZipReader::entries() const
	{
		...
		do
		{
			...
			if(unzGetCurrentFileInfo64(m_handle, &info, nullptr, 0, nullptr, 0, nullptr, 0) != UNZ_OK)
				continue;
			
			std::string nameBuff(info.size_filename, '\0');
			
			if(unzGetCurrentFileInfo64(m_handle, &info, nameBuff.data(), static_cast<unsigned int>(nameBuff.size()), nullptr, 0, nullptr, 0) != UNZ_OK)
				continue;
	...
	{
		if(!os.good())
			throw MAKE_EXCEPTION(ZipException, "ERROR : Output stream is not in a good state for archive \"" + m_path.string() + "\".");
